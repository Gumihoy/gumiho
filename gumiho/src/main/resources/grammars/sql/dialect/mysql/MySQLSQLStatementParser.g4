parser grammar MySQLSQLStatementParser;

options {tokenVocab=MySQLSQLStatementLexer;}
...
createTableStatement
    : CREATE TEMPORARY? TABLE ifNotExists? nameIdentifier (tableElementsParen=LEFT_PAREN? tableElement (COMMA tableElement)* RIGHT_PAREN?)?
		(tableOption (COMMA? tableOption)*)? iPartitionBy? keyViolate? (AS? iSelectQuery)?
    ;
...
iPartitionBy
	: PARTITION BY LINEAR? HASH LEFT_PAREN columns+=expr RIGHT_PAREN (PARTITIONS partitionsNum=expr)? iSubPartitionBy?
		(LEFT_PAREN partitionDefinition (COMMA partitionDefinition)* RIGHT_PAREN)?                                      #partitionByHash
	| PARTITION BY LINEAR? KEY (ALGORITHM EQUALS_OP algorithm=expr)?
		LEFT_PAREN columns+=expr (COMMA columns+=expr)* RIGHT_PAREN (PARTITIONS partitionsNum=expr)?
		iSubPartitionBy? (LEFT_PAREN partitionDefinition (COMMA partitionDefinition)* RIGHT_PAREN)?                     #partitionByKey
	| PARTITION BY LINEAR? RANGE LEFT_PAREN columns+=expr RIGHT_PAREN (PARTITIONS partitionsNum=expr)? iSubPartitionBy?
        (LEFT_PAREN partitionDefinition (COMMA partitionDefinition)* RIGHT_PAREN)?                                      #partitionByRange
	| PARTITION BY LINEAR? RANGE COLUMNS LEFT_PAREN columns+=expr (COMMA columns+=expr)* RIGHT_PAREN
		(PARTITIONS partitionsNum=expr)?
		iSubPartitionBy? (LEFT_PAREN partitionDefinition (COMMA partitionDefinition)* RIGHT_PAREN)?          		    #partitionByRangeColumns
	| PARTITION BY LINEAR? LIST LEFT_PAREN columns+=expr RIGHT_PAREN (PARTITIONS partitionsNum=expr)?
		iSubPartitionBy? (LEFT_PAREN partitionDefinition (COMMA partitionDefinition)* RIGHT_PAREN)?                     #partitionByList
	| PARTITION BY LINEAR? LIST COLUMNS LEFT_PAREN columns+=expr (COMMA columns+=expr)* RIGHT_PAREN
		(PARTITIONS partitionsNum=expr)?
		iSubPartitionBy? (LEFT_PAREN partitionDefinition (COMMA partitionDefinition)* RIGHT_PAREN)?          		    #partitionByListColumns
	;
iSubPartitionBy
	: SUBPARTITION BY LINEAR? HASH LEFT_PAREN columns+=expr RIGHT_PAREN (SUBPARTITIONS subpartitionsNum=expr)?          #subPartitionByHash
	| SUBPARTITION BY LINEAR? KEY (ALGORITHM EQUALS_OP algorithm=expr)?
		LEFT_PAREN columns+=expr (COMMA columns+=expr)* RIGHT_PAREN (SUBPARTITIONS subpartitionsNum=expr)?              #subPartitionByKey
	;
partitionDefinition
    : PARTITION nameIdentifier iPartitionValues? partitionOption*
...
iPartitionValues
	...
	| VALUES LESS THAN MAXVALUE                                             #partitionValuesLessThanMaxValue
	| VALUES IN LEFT_PAREN expr (COMMA expr)* RIGHT_PAREN                   #partitionValuesIn
	;
subPartitionDefinition
    : SUBPARTITION nameIdentifier partitionOption*
    ;

partitionOption
    : STORAGE? ENGINE EQUALS_OP? engineName                 #partitionOptionEngine
    | COMMENT EQUALS_OP? expr                               #partitionOptionComment
    | DATA DIRECTORY EQUALS_OP? expr                        #partitionOptionDataDirectory
	| INDEX DIRECTORY EQUALS_OP? expr                       #partitionOptionIndexDirectory
    | MAX_ROWS EQUALS_OP? expr                              #partitionOptionMaxRows
    | MIN_ROWS EQUALS_OP? expr                              #partitionOptionMinRows
    | TABLESPACE EQUALS_OP? nameIdentifier                  #partitionOptionTablespace
    | NODEGROUP EQUALS_OP? nameIdentifier                   #partitionOptionNodeGroup
    ;
...
// ----- alter table ---------------------
alterTableStatement
    : ALTER intimeAction? IGNORE? TABLE nameIdentifier alterTableItem (COMMA alterTableItem)* iPartitionBy?
    ;
...
alterTableItem
    ...
    | ADD PARTITION partitionDefinition                                                                                 #alterTableAddPartitionAction
    ...
    | REORGANIZE PARTITION expr (COMMA expr)*
        INTO LEFT_PAREN partitionDefinition (COMMA partitionDefinition)* RIGHT_PAREN                                    #alterTableReorganizePartitionAction
...
alterTableColumnActionProperty
	: FIRST
	| AFTER nameIdentifier
	;
alterColumnAction
...
renameTableStatement
    : RENAME TABLE
    renameTableClause (COMMA renameTableClause)*
    ;
