lexer grammar MySQLSQLStatementLexer;

channels { COMMENTS, ERRORCHANNEL }

// SKIP
SPACES: [ \t\r\n\u3000]+ -> skip;       // \u3000 is Chinese space
SPEC_MYSQL_COMMENT:                  '/*!' .+? '*/' -> channel(COMMENTS);
...
PERIOD:                                 '.';
// Constructors symbols
LEFT_PAREN:                             '(' | '\uFF08'; // ff08 chinese (
RIGHT_PAREN:                            ')' | '\uFF09'; // ff09 chinese )
//LEFT_BRACKET:                           '[';
//RIGHT_BRACKET:                          ']';
LEFT_BRACE:                             '{';
RIGHT_BRACE:                            '}';
...
DELIMITED_ID:                        DQUOTA_STRING;
REVERSE_QUOTE_ID:                    '`' (~'`')+ '`';

// literals Primitives
CHAR_STRING:                         SQUOTA_STRING;
NCHAR_STRING:                        'N' SQUOTA_STRING;
STRING_CHARSET_NAME:                 '_' CHARSET_NAME;

// 数字
UNSIGNED_INTEGER:                    DEC_DIGIT+;      // 正整数
APPROXIMATE_NUM_LIT:                 UNSIGNED_INTEGER? '.' UNSIGNED_INTEGER
                                     | UNSIGNED_INTEGER? '.' UNSIGNED_INTEGER EXPONENT_NUM_PART
                                     | UNSIGNED_INTEGER EXPONENT_NUM_PART;
...
// join
INNER_JOIN:                         INNER SPACES JOIN;
CROSS_JOIN:                         CROSS SPACES JOIN;
NATURAL_JOIN:                       NATURAL SPACES JOIN;
NATURAL_INNER_JOIN:                 NATURAL SPACES INNER SPACES JOIN;

LEFT_JOIN:                          LEFT SPACES JOIN;
LEFT_OUTER_JOIN:                    LEFT SPACES OUTER SPACES JOIN;
NATURAL_LEFT_JOIN:                  NATURAL SPACES LEFT SPACES JOIN;
NATURAL_LEFT_OUTER_JOIN:            NATURAL SPACES LEFT SPACES OUTER SPACES JOIN;

RIGHT_JOIN:                         RIGHT SPACES JOIN;
RIGHT_OUTER_JOIN:                   RIGHT SPACES OUTER SPACES JOIN;
NATURAL_RIGHT_JOIN:                 NATURAL SPACES RIGHT SPACES JOIN;
NATURAL_RIGHT_OUTER_JOIN:           NATURAL SPACES RIGHT SPACES OUTER SPACES JOIN;
...
fragment SIMPLE_LETTER:              [A-Z] | '\u4E00'..'\u9FA5' | '\uF900'..'\uFA2D';
