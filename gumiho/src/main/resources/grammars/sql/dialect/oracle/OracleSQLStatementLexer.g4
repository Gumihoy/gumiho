lexer grammar OracleSQLStatementLexer;
...
APPROX_COUNT_DISTINCT_AGG:      'APPROX_COUNT_DISTINCT_AGG';
APPROX_COUNT_DISTINCT_DETAIL:   'APPROX_COUNT_DISTINCT_DETAIL';
APPROX_MEDIAN:                  'APPROX_MEDIAN';
APPROX_PERCENTILE:              'APPROX_PERCENTILE';
APPROX_PERCENTILE_AGG:          'APPROX_PERCENTILE_AGG';
APPROX_PERCENTILE_DETAIL:       'APPROX_PERCENTILE_DETAIL';
...
ORADATA:                      'ORADATA';
// ORADEBUG:                     'ORADEBUG';
ORA_DM_PARTITION_NAME:          'ORA_DM_PARTITION_NAME';
...
// PENDING:                      'PENDING';
PERCENT_BULK_EXCEPTIONS:      '%BULK_EXCEPTIONS';
PERCENT_BULK_ROWCOUNT:        '%BULK_ROWCOUNT';
PERCENT_CHARSET:              '%CHARSET';
PERCENT_FOUND:                '%FOUND';
...
SUBPARTITIONSET:                'SUBPARTITIONSET';
...
//SUPPLEMENTAL_LOG_DATA:        'SUPPLEMENTAL_LOG_DATA';
SUSPEND:                      'SUSPEND';
...
//TIMES:                        'TIMES';
TIME:                         'TIME';
TIME_TZ_UNCONSTRAINED:        'TIME_TZ_UNCONSTRAINED';
...
// TIV_SSF:                      'TIV_SSF';
// TO_ACLID:                     'TO_ACLID';
TO_APPROX_COUNT_DISTINCT:       'TO_APPROX_COUNT_DISTINCT';
TO_APPROX_PERCENTILE:           'TO_APPROX_PERCENTILE';
...
START_CMD
    ...
    // 'start' SPACE ( ~( '\r' | '\n') )* (NEWLINE|EOF)
    : 'sta' SPACE ( ~('\r' | '\n') )* (NEWLINE|EOF)
    // TODO Single @ conflicts with a database link ownedBy, like employees@remote
    // | '@' ( ~('\r' | '\n') )* (NEWLINE|EOF)
    | '@@' ( ~('\r' | '\n') )* (NEWLINE|EOF)
    ;
...
QCHAR_STRING:                       'Q' (QS_ANGLE|QS_BRACE | QS_BRACK |QS_PAREN|QS_CARET|QS_DOUBLE_QUOTE|QS_EXCLAMATION|QS_ï);
NQCHAR_STRING:                      'NQ' (QS_ANGLE|QS_BRACE | QS_BRACK |QS_PAREN|QS_CARET|QS_DOUBLE_QUOTE|QS_EXCLAMATION|QS_ï);

// 数字
UNSIGNED_INTEGER:                   UNSIGNED_INTEGER_FRAGMENT;      // 正整数
APPROXIMATE_NUM_LIT:                FLOAT_FRAGMENT ('E' ('+'|'-')? UNSIGNED_INTEGER_FRAGMENT)?;
BINARY_FLOAT_LIT:                   APPROXIMATE_NUM_LIT 'F';
BINARY_DOUBLE_LIT:                  APPROXIMATE_NUM_LIT 'D';
...
FULL_JOIN:                          FULL SPACES JOIN;
FULL_OUTER_JOIN:                    FULL SPACES OUTER SPACES JOIN;
NATURAL_FULL_JOIN:                  NATURAL SPACES FULL SPACES JOIN;
NATURAL_FULL_OUTER_JOIN:            NATURAL SPACES FULL SPACES OUTER SPACES JOIN;
...
fragment FLOAT_FRAGMENT:            UNSIGNED_INTEGER_FRAGMENT* '.'? UNSIGNED_INTEGER_FRAGMENT+;
...
fragment QS_CARET:                  QUOTE '^' .*? '^' QUOTE ;
fragment QS_DOUBLE_QUOTE:           QUOTE '"' .*? '"' QUOTE ;
fragment QS_EXCLAMATION:            QUOTE '!' .*? '!' QUOTE ;
fragment QS_ï:                      QUOTE '\u00ef' .*? '\u00ef' QUOTE ;
fragment QS_OTHER_CH:               ~('<' | '{' | '[' | '(' | ' ' | '\t' | '\n' | '\r');


fragment SQUOTA_STRING:             '\'' ('\\'. | '\'\'' | NEWLINE | ~('\'' | '\\'))* '\'';
