parser grammar PostgreSQLSQLStatementParser;

options {tokenVocab=PostgreSQLSQLStatementLexer;}

parse
    : ((statement | command | expr) SEMI?)* EOF
    ;
...
createDatabaseStatement
	: CREATE DATABASE databaseName=nameIdentifier userSysClause? userSystemClause? controlfileReuseClause?
		maxdatafilesClause? maxinstancesClause? characterSet? nationalCharacterSet? setDefaultTablespaceClause?

        databaseLogFileClause? databaseMaxLogFilesClause? databaseMaxLogMembersClause? databaseMaxLogHistoryClause?
        databaseArchiveLogTypeClause? databaseForceLoggingClause?

        extentManagementClause? dataFileClause? sysauxClause? defaultTablespace? defaultTempTablespace? undoTablespace?

        setTimeZoneClause? userDataTablespaceClause? enablePluggableDatabase?
		SEMI?
	;
userSysClause
...
extentManagementClause
	: extentManagementLocalClause | extentManagementLocalAutoAllocateClause | extentManagementLocalUniformClause
	;
...
tablespaceDatafileClauses
	...
	;
databaseLogFileClause
	: LOGFILE (g1=GROUP UNSIGNED_INTEGER)? fileSpecification (COMMA (g2=GROUP UNSIGNED_INTEGER)? fileSpecification)*
	;
databaseMaxLogFilesClause
	: MAXLOGFILES UNSIGNED_INTEGER
	;
databaseMaxLogMembersClause
	: MAXLOGMEMBERS UNSIGNED_INTEGER
	;
databaseMaxLogHistoryClause
	: MAXLOGHISTORY UNSIGNED_INTEGER
	;
databaseArchiveLogTypeClause
	: ARCHIVELOG | NOARCHIVELOG
	;
databaseForceLoggingClause
	: FORCE LOGGING
	;
...
createDatabaseLinkStatement
	: CREATE SHARED? PUBLIC? DATABASE LINK dbLinkName=nameIdentifier
        connectToCurrentUser? connectToIdentifiedBy? dblinkAuthentication? dblinkUsing?
        SEMI?
	;
...
connectToIdentifiedBy
	: CONNECT TO user=IDENTIFIER IDENTIFIED BY password=IDENTIFIER dblinkAuthentication?
	;
dblinkAuthentication
	: AUTHENTICATED BY user=IDENTIFIER IDENTIFIED BY password=IDENTIFIER
	;
dblinkUsing
	: USING connectString=literal
	;
...
createFunctionStatement
    : CREATE orReplace? editionableType? FUNCTION name=nameIdentifier (LEFT_PAREN parameterDeclaration (COMMA parameterDeclaration)* RIGHT_PAREN)?
      RETURN dataType functionOption* (asType (declareSection* body SEMI| callSpec))?
    ;
...
createIndexStatement
	...
	;
alterIndexStatement
	: ALTER INDEX nameIdentifier alterIndexStatementOption? alterIndexStatementProperty*
	;
dropIndexStatement
...
alterIndexStatementOption
	: rebuildClause
	| parametersClause
	| alterIndexCompileOption
	| alterIndexEnableOption
	| alterIndexDisableOption
	| alterIndexUnusableOption
	| alterIndexVisibleOption
	| alterIndexInvisibleOption
	| renameToClause
	| alterIndexCoalesceOption
	| alterIndexMonitoringUsageOption
	| alterIndexNoMonitoringUsageOption
	| alterIndexUpdateBlockReferencesOption
	| alterIndexPartitioning
	;
alterIndexStatementProperty
...
shrinkClause
	...
	;
rebuildClause
	: REBUILD (PARTITION|SUBPARTITION|REVERSE |NOREVERSE)? expr?
		(parallelClause|tableSpaceClause|parametersClause|physicalAttributesClause| indexCompression|loggingClause| partialIndexClause)?
        | ONLINE
	;
alterIndexCompileOption
	: COMPILE
...
alterIndexPartitioning
	...
	| alterIndexRenameSubPartition
	| alterIndexDropPartition
	| alterIndexSplitPartition
	| alterIndexCoalescePartition
	| alterIndexModifySubpartition
	;
...
alterIndexModifyPartitionAction
	: MODIFY PARTITION nameIdentifier
      (deallocateUnusedClause
      | allocateExtentClause|physicalAttributesClause| loggingClause|indexCompression
      | COALESCE CLEANUP?
...
createProcedureStatement
    : CREATE orReplace? editionableType? PROCEDURE nameIdentifier (LEFT_PAREN parameterDeclaration (COMMA parameterDeclaration)* RIGHT_PAREN)?
...
createSequenceStatement
    : CREATE SEQUENCE  ifNotExists? sequenceName=nameIdentifier createSequenceOption*
    ;
...
createTableStatement
    : CREATE tableScope? TABLE ifNotExists? tableName=nameIdentifier
		(OF ofDataType=dataType)? objectTableSubstitution? (LEFT_PAREN tableElement (COMMA tableElement)* RIGHT_PAREN)?
        collationExpr? commitActionDefinition? commitActionRows? oidClause? oidIndexClause?
...
alterTableStatement
    : ALTER TABLE nameIdentifier memoptimizeReadClause? alterTableItem* alterIotClauses*
    ;
...
columnDefinition
	: columnName=nameIdentifier dataType? collateExpr? SORT? visibleType? (defaultClause | identityClause)? encryptClause? columnConstraint*
	;
...
virtualColumnDefinition
	: nameIdentifier dataType? collationExpr? visibleType? generatedAlways? AS LEFT_PAREN expr RIGHT_PAREN VIRTUAL? evaluationEditionClause? unusableEditionsClause? columnConstraint*
	;
...
indexOrgOverflowClause
	: (INCLUDING expr)? OVERFLOW segmentAttributesClause*
	;
iSupplementalLog
...
alterIotClauses
	...
	// indexOrgTableClause : indexOrgTableClauseItem* indexOrgOverflowClause?
	| indexOrgTableClauseItem
	| indexOrgOverflowClause
...
addOverflowClause
	: ADD OVERFLOW segmentAttributesClause* LEFT_PAREN RIGHT_PAREN
	;
alterMappingTableClause
...
alterTableColumnAction
	...
	| alterTalbeModifyCollectionRetrievalAction
...
alterTableAddColumnAction
	...
	;
alterTableModifyColumnAction
	: MODIFY (LEFT_PAREN iColumnDefinition (COMMA iColumnDefinition)* RIGHT_PAREN)? modifyColSubstitutable?
	;
modifyColSubstitutable
	: COLUMN expr NOT? SUBSTITUTABLE AT ALL LEVELS FORCE?
	;
iAlterTableDropColumnAction
...
iAlterTableConstraintAction
	...
	| DROP PRIMARY KEY CASCADE? ((KEEP | DROP) INDEX)? ONLINE?                                  #alterTableDropPrimaryKeyConstraintAction
	| DROP UNIQUE LEFT_PAREN expr (COMMA expr)* RIGHT_PAREN
              CASCADE? ((KEEP | DROP) INDEX)? ONLINE?                                           #alterTableDropUniqueConstraintAction
...
alterTableModifyDefaultAttrsAction
	: MODIFY DEFAULT ATTRIBUTES iForPartition? readOnlyClause? indexingClause? segmentAttributesClause?
		tableCompression? iInMemoryClause? pctthresholdClause? iPrefixCompression? alterOverflowClause
...
alterTableModifySubPartitionAction
	: MODIFY SUBPARTITION nameIdentifier
	;
alterTableModifySubPartitionForAction
	: MODIFY SUBPARTITION FOR LEFT_PAREN nameIdentifier (COMMA nameIdentifier)* RIGHT_PAREN
	;

alterTableMovePartitionAction
	: MODIFY PARTITION nameIdentifier (MAPPING TABLE)?  ONLINE?
	;
alterTableMovePartitionForAction
	: MODIFY PARTITION FOR LEFT_PAREN nameIdentifier (COMMA nameIdentifier)* RIGHT_PAREN
	;
...
alterTableAddSubPartitionAction
...
dependentTablesClauseItem
	...
	;
alterTableCoalesceTablePartition
	: COALESCE PARTITION  parallelClause? updateIndexClause? allowDisallowClustering?
	;
alterTableCoalesceTableSubpartition
	: COALESCE SUBPARTITION nameIdentifier updateIndexClause? allowDisallowClustering?
	;
...
alterTableDropSubpartitionAction
	: DROP (SUBPARTITION | SUBPARTITIONS) dropSubpartitionActionItem (COMMA dropSubpartitionActionItem)*
	;
dropSubpartitionActionItem
	: nameIdentifier
	| FOR LEFT_PAREN names+=nameIdentifier (COMMA names+=nameIdentifier)
	;
...
iAlterTableSplitPartitionAction
	: SPLIT PARTITION name=nameIdentifier AT LEFT_PAREN atItems+=expr (COMMA atItems+=expr)* RIGHT_PAREN
		(INTO LEFT_PAREN partitionDefinition COMMA partitionDefinition RIGHT_PAREN)?
        splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
        parallelClause? allowDisallowClustering? ONLINE?                                                                #alterTableSplitPartitionaAtAction
	| SPLIT PARTITION name=nameIdentifier VALUES LEFT_PAREN values+=expr (COMMA values+=expr)* RIGHT_PAREN
        (INTO LEFT_PAREN partitionDefinition COMMA partitionDefinition RIGHT_PAREN)?
        splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
		parallelClause? allowDisallowClustering? ONLINE?                                                                #alterTableSplitPartitionaValuesAction
	| SPLIT PARTITION name=nameIdentifier INTO INTO LEFT_PAREN partitionDefinition (COMMA partitionDefinition RIGHT_PAREN)*
         splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
         parallelClause? allowDisallowClustering? ONLINE?                                                               #alterTableSplitPartitionaIntoAction
	| SPLIT PARTITION FOR LEFT_PAREN nameIdentifier (COMMA nameIdentifier)* RIGHT_PAREN
        ...
        (INTO LEFT_PAREN partitionDefinition COMMA partitionDefinition RIGHT_PAREN)?
        splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
        parallelClause? allowDisallowClustering? ONLINE?                                                                #alterTableSplitPartitionaForAtAction
    | SPLIT PARTITION FOR LEFT_PAREN nameIdentifier (COMMA nameIdentifier)* RIGHT_PAREN
        VALUES LEFT_PAREN values+=expr (COMMA values+=expr)* RIGHT_PAREN
        (INTO LEFT_PAREN partitionDefinition COMMA partitionDefinition RIGHT_PAREN)?
        splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
        parallelClause? allowDisallowClustering? ONLINE?                                                                #alterTableSplitPartitionaForValuesAction
    | SPLIT PARTITION FOR LEFT_PAREN nameIdentifier (COMMA nameIdentifier)* RIGHT_PAREN
        INTO INTO LEFT_PAREN partitionDefinition (COMMA partitionDefinition RIGHT_PAREN)*
		splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
        parallelClause? allowDisallowClustering? ONLINE?                                                                #alterTableSplitPartitionaForIntoAction
	;

splitNestedTablePart
	: NESTED TABLE expr INTO LEFT_PAREN partitionDefinition COMMA partitionDefinition splitNestedTablePart? RIGHT_PAREN splitNestedTablePart?
	;


iAlterTableSubSplitPartitionAction
	: SPLIT SUBPARTITION name=nameIdentifier AT LEFT_PAREN atItems+=expr (COMMA atItems+=expr)* RIGHT_PAREN
		(INTO LEFT_PAREN partitionDefinition COMMA partitionDefinition RIGHT_PAREN)?
        splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
        parallelClause? allowDisallowClustering? ONLINE?                                                                #alterTableSplitSubPartitionaAtAction
	| SPLIT SUBPARTITION name=nameIdentifier VALUES LEFT_PAREN values+=expr (COMMA values+=expr)* RIGHT_PAREN
        (INTO LEFT_PAREN partitionDefinition COMMA partitionDefinition RIGHT_PAREN)?
        splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
		parallelClause? allowDisallowClustering? ONLINE?                                                                #alterTableSplitSubPartitionaValuesAction
	| SPLIT SUBPARTITION name=nameIdentifier INTO INTO LEFT_PAREN partitionDefinition (COMMA partitionDefinition RIGHT_PAREN)*
         splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
         parallelClause? allowDisallowClustering? ONLINE?                                                               #alterTableSplitSubPartitionaIntoAction
	| SPLIT SUBPARTITION FOR LEFT_PAREN nameIdentifier (COMMA nameIdentifier)* RIGHT_PAREN
		AT LEFT_PAREN atItems+=expr (COMMA atItems+=expr)* RIGHT_PAREN
        (INTO LEFT_PAREN partitionDefinition COMMA partitionDefinition RIGHT_PAREN)?
        splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
        parallelClause? allowDisallowClustering? ONLINE?                                                                #alterTableSplitSubPartitionaForAtAction
    | SPLIT SUBPARTITION FOR LEFT_PAREN nameIdentifier (COMMA nameIdentifier)* RIGHT_PAREN
        VALUES LEFT_PAREN values+=expr (COMMA values+=expr)* RIGHT_PAREN
        (INTO LEFT_PAREN partitionDefinition COMMA partitionDefinition RIGHT_PAREN)?
        splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
        parallelClause? allowDisallowClustering? ONLINE?                                                                #alterTableSplitSubPartitionaForValuesAction
    | SPLIT SUBPARTITION FOR LEFT_PAREN nameIdentifier (COMMA nameIdentifier)* RIGHT_PAREN
        INTO INTO LEFT_PAREN partitionDefinition (COMMA partitionDefinition RIGHT_PAREN)*
		splitNestedTablePart? filterCondition? dependentTablesClause? updateIndexClause?
        parallelClause? allowDisallowClustering? ONLINE?                                                                #alterTableSplitSubPartitionaForIntoAction
	;
...
iAlterTableMergePartitionsAction
	: MERGE PARTITIONS alterTablePartitionItem (COMMA alterTablePartitionItem)* (INTO partitionDefinition)? filterCondition?
	    dependentTablesClause? updateIndexClause? parallelClause? allowDisallowClustering?                              #alterTableMergePartitionsAction
	| MERGE PARTITIONS alterTablePartitionItem TO alterTablePartitionItem (INTO partitionDefinition)? filterCondition?
		dependentTablesClause? updateIndexClause? parallelClause? allowDisallowClustering?                              #alterTableMergePartitionsToAction
	;
...
iAlterTableMergeSubPartitionsAction
	: MERGE SUBPARTITIONS alterTablePartitionItem (COMMA alterTablePartitionItem)* (INTO subPartitionDefinition)?
         filterCondition? dependentTablesClause? updateIndexClause? parallelClause? allowDisallowClustering?            #alterTableMergeSubPartitionsAction
	| MERGE SUBPARTITIONS alterTablePartitionItem TO alterTablePartitionItem (INTO subPartitionDefinition)?
		filterCondition? dependentTablesClause? updateIndexClause? parallelClause? allowDisallowClustering?             #alterTableMergeSubPartitionsToAction
	;
...
iAlterTableExchangePartitionAction
	...
	| EXCHANGE PARTITION FOR LEFT_PAREN RIGHT_PAREN WITH TABLE nameIdentifier ((INCLUDING | EXCLUDING) INDEXES)?
...
updateIndexesClauseItem
	: expr LEFT_PAREN (partitionDefinition|subPartitionDefinition) RIGHT_PAREN
	;

alterTableMoveTableAction
	: MOVE filterCondition? ONLINE? segmentAttributesClause* tableCompression indexOrgTableClause?
         (lobStorageClause | varrayColProperty)* parallelClause? allowDisallowClustering?
         (UPDATE INDEXES LEFT_PAREN alterTableMoveTableActionUpdateIndexesItem (COMMA alterTableMoveTableActionUpdateIndexesItem)* RIGHT_PAREN)?
	;
alterTableMoveTableActionUpdateIndexesItem
	: expr segmentAttributesClause*
	;
...
// Type DDLs
createTypeStatement
    : CREATE orReplace? editionableType? TYPE nameIdentifier FORCE? (OID literal)?
...
replaceTypeClause
    : REPLACE invokerRightsClause? AS OBJECT LEFT_PAREN
    ;
...
alter_method_element
    : (ADD|DROP) (mapOrderFunctionDeclaration | subProgramDeclaration)
    ;

alter_attribute_definition
    : (ADD | MODIFY | DROP) ATTRIBUTE (attributeDefinition | LEFT_PAREN attributeDefinition (COMMA attributeDefinition)* RIGHT_PAREN)
    ;


alter_collection_clauses
    : MODIFY (LIMIT expr | ELEMENT TYPE dataType)
    ;
...
dependent_exceptions_part
    : FORCE? EXCEPTIONS INTO nameIdentifier
    ;
...
// -------------------------------- View DDLs Start
createViewStatement
    : CREATE orReplace? forceType? editionableType? VIEW nameIdentifier
...
alterViewAction
	...
	| DROP UNIQUE LEFT_PAREN expr (COMMA expr)* RIGHT_PAREN     #alterViewCompileAction
    | COMPILE                                                   #alterViewAddTableAction
...
tableReferenceBasic
	: ONLY LEFT_PAREN tableName=nameIdentifier objectNameTableReferenceOption?
      		sampleClause? RIGHT_PAREN flashbackQueryClause* iPivotClause?  (AS? alias=nameIdentifier)?                  #objectNameTableReference
	| tableName=nameIdentifier objectNameTableReferenceOption? sampleClause?
			flashbackQueryClause* iPivotClause?  (AS? alias=nameIdentifier)?                                            #objectNameTableReference
...
anyDataType
	: SYS_COMMA_ANYDATA             #sysAnyDataDataType
	| SYS_COMMA_ANYTYPE             #sysAnyTypeDataType
	| SYS_COMMA_ANYDATASET          #sysAnyDataSetDataType
	;
xmlDataType
	: XMLTYPE                       #xmlTypeDataType
	| URITYPE                       #uriTypeDataType
	;
iJsonDataType
	: JSON                          #jsonDataType
	;
spatialDataType
	: SDO_GEOMETRY                  #sdoGeometryDataType
	| SDO_TOPO_GEOMETRY             #sdoTopoGeometryDataType
	| SDO_GEORASTER                 #sdoGeorasterDataType
	;
mediaDataType
	: ORDAUDIO                      #oRDAUDIODataType
	| ORDIMAGE                      #oRDIMAGEDataType
	| ORDVIDEO                      #oRDVIDEODataType
	| ORDDOC                        #oRDDOCDataType
	| ORDDICOM                      #oRDDICOMDataType

	| SI_STILLIMAGE                 #siStillimageDataType
    | SI_AVERAGECOLOR               #siAveragecolorDataType
    | SI_POSITIONALCOLOR            #siPositionalcolorDataType
    | SI_COLORHISTOGRAM             #siColorhistogramDataType
    | SI_TEXTURE                    #siTextureDataType
    | SI_FEATURELIST                #siFeaturelistDataType
    | SI_COLOR                      #siColorDataType
	;
...
literal
	...
	| TIMESTAMP value=expr (AT TIME ZONE atTimeZone=expr)?                                  #timestampLiteral
	| INTERVAL value=expr
...
nullExpr
	...
	;
sequenceExpr
	: sequence=nameIdentifier PERIOD (CURRVAL | NEXTVAL)
	;
caseExpr
...
intervalExpr
	: LEFT_PAREN expr MINUS_SIGN expr RIGHT_PAREN (DAY|YEAR) (LEFT_PAREN precision=expr RIGHT_PAREN)?
		TO (SECOND|MONTH) (LEFT_PAREN precision=expr RIGHT_PAREN)?
	;
...
columnConstraint
    ...
    | (CONSTRAINT constraintName=nameIdentifier)? REFERENCES referenced=nameIdentifier
        (LEFT_PAREN expr (COMMA expr)* RIGHT_PAREN)?
		(ON DELETE (CASCADE | SET NULL))? iConstraintState*                                                             #referencesColumnConstraint
...
physicalProperty
	: deferredSegmentCreation? segmentAttributesClause+ inMemoryTableClause? ilmClause?                                 #physicalPropertySegmentAttributesClause
	| deferredSegmentCreation? ORGANIZATION HEAP segmentAttributesClause* heapOrgTableClause                            #physicalPropertyOrganizationHeapClause
	| deferredSegmentCreation? ORGANIZATION INDEX segmentAttributesClause* indexOrgTableClause                          #physicalPropertyOrganizationIndexClause
	| deferredSegmentCreation? ORGANIZATION EXTERNAL externalTableClause                                                #physicalPropertyOrganizationExternalClause
	| CLUSTER cluster=expr LEFT_PAREN columns+=expr (COMMA columns+=expr)* RIGHT_PAREN                                  #physicalPropertyClusterClause
	;
...
partitionByList
	...
	;
partitionByReference
	: PARTITION BY REFERENCE LEFT_PAREN constraint RIGHT_PAREN (LEFT_PAREN partitionDefinition (COMMA partitionDefinition)* RIGHT_PAREN)?
	;
partitionBySystem
