 ...
 */
package com.aliyun.gumiho.sql.bvt.translate.oracle2.postgresql.version9_6.ddl.tyepbody.create;
 ...
 */
public class OracleCreateTypeBodyTest_0_remove_quote {

    @Test
    public void test1() {
        String sql = "create or replace TYPE BODY           \"JSON_LIST\" as\n" +
        ...
        Assert.assertEquals("CREATE OR REPLACE TYPE BODY JSON_LIST AS\n" +
                ...
                "\t) AS\n" +
                "\t\tindx PLS_INTEGER;\n" +
                "\t\tinsert_value json_value DEFAULT NVL(elem, json_value);\n" +
                "\t\tBEGIN\n" +
                ...
                "\t) AS\n" +
                "\t\tobjlist JSON;\n" +
                "\t\tjp json_list DEFAULT json_ext.parsePath(json_path, base);\n" +
                "\t\tBEGIN\n" +
                "\t\t\tWHILE (jp.head().get_number() > self.count) LOOP\n" +
                "\t\t\t\tself.append(json_value());\n" +
                "\t\t\tEND LOOP;\n" +
                ...
                "\t\tbase NUMBER DEFAULT 1\n" +
                "\t) AS\n" +
                "\t\tobjlist JSON DEFAULT json(self);\n" +
                "\t\tBEGIN\n" +
                "\t\t\tjson_ext.remove(objlist, json_path, base);\n" +
                "\t\t\tself := objlist.get_values;\n" +
                "\t\tEND path_remove;\n" +
